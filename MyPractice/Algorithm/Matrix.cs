namespace Algorithm
{
    public class Matrix
    {
        ...
        private static int RecursiveMemoMatrix(int[] p, int i, int j)
        {
            if (i == j)
            {
                memo[i, i] = 0;
        ...
        public static int MemoizedMatrix(int[] p, int i, int j)
        {
            memo = new int[j, j];
            for (int k = 0; k < j; k++)
                for (int t = 0; t < j; t++)
        ...
        public static int DynamicMatrix(int[] p, int i, int j)
        {
            int[,] m = new int[j, j];
