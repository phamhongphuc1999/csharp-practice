...
- `Bundler`: A node(block builder) that bundles multiple `UserOperation` and create an `EntryPoint.handleOps()` transaction. Note that not all block-bundlers on the network are required to be bundlers.
